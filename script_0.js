...
function sendNotification(title, body){
    ...
    if(Notification.permission==='granted'){
      if('serviceWorker' in navigator && navigator.serviceWorker.controller){
        navigator.serviceWorker.ready.then(reg=>{
          reg.showNotification(title, { body, icon:'icon-192.png', badge:'icon-192.png', tag:'life-planner', renotify:true });
        }).catch(()=>{
  ...
  }catch(e){ /* notifications unavailable, ignore silently */ }
}
...
function requestNotifyPermission(){
  if(!('Notification' in window)){ toast('⚠️ مرورگرت از نوتیفیکیشن پشتیبانی نمی‌کنه'); return; }
  if(Notification.permission==='granted'){ toast('✅ نوتیفیکیشن از قبل فعاله'); renderPomoNotifyStatus(); return; }
  if(Notification.permission==='denied'){ toast('⚠️ نوتیفیکیشن مسدوده — باید از تنظیمات مرورگر/گوشی فعالش کنی'); return; }
  Notification.requestPermission().then(()=>{ renderPomoNotifyStatus(); sendNotification('🔔 نوتیفیکیشن فعال شد!', 'از این به بعد وقتی پومودورو تموم بشه یا کوئست جدید بیاد، خبرت می‌کنیم'); });
}
function renderPomoNotifyStatus(){
  const el = document.getElementById('pomoNotifyStatus');
  if(!el) return;
  const p = notifyPermissionStatus();
  el.textContent = p==='granted' ? '✅ نوتیفیکیشن فعاله' : p==='denied' ? '⛔ نوتیفیکیشن مسدود شده (از تنظیمات گوشی/مرورگر بازش کن)' : '🔔 نوتیفیکیشن هنوز فعال نشده';
}
function toast(msg){
...
function showView(v){
  ...
  if(v==='city'){
    citySunMoonInited=false;
    setTimeout(renderCityScene,30);
    setTimeout(()=>{
      if(!citySoundOn){
        citySoundOn=true;
  ...
  if(v==='settings')setTimeout(renderSettings,30);
  if(v!=='city'&&citySoundOn){
    citySoundOn=false;
    stopCityAmbience();
    const b=document.getElementById('citySoundBtn');
    if(b)b.textContent='🔇';
...
function updateCelestialPosition(instant){
  ...
  if(instant || !citySunMoonInited){
    rig.style.transition = 'none';
    citySunMoonInited = true;
...
function renderCityScene(){
  ...
  frame.classList.toggle('city-rain-active', cityRainOn);
  ...
  document.getElementById('cityHint').textContent = citySoundOn ? '🔊 صدای طبیعت داره پخش می‌شه' : '🔇 صدا قطعه — برای پخش بزن';
...
let cityRainOn = false;
function toggleCityRain(){
  cityRainOn = !cityRainOn;
  document.getElementById('cityRainBtn').textContent = cityRainOn ? '☔' : '🌦️';
  renderCityScene();
  updateCityRainAudio();
}

/* ambient nature sound (generative, no external audio files) */
let cityAudioCtx = null, cityAudioNodes = null, citySoundOn = false, cityChirpTimer = null;
let cityRainNoiseSrc = null, cityRainGainNode = null;
let cityRiverNoiseSrc = null, cityRiverGainNode = null, cityWindNoiseSrc = null, cityWindGainNode = null;
...
function birdChirp(ctx){
  ...
  [0, 0.11+Math.random()*0.05].forEach((t,i)=>{
    const osc=ctx.createOscillator(), g=ctx.createGain(), f=ctx.createBiquadFilter();
    osc.type='sine';
    const f0=base+(i?300+Math.random()*240:0);
    osc.frequency.setValueAtTime(f0,ctx.currentTime+t);
    osc.frequency.exponentialRampToValueAtTime(f0*1.35,ctx.currentTime+t+0.055);
    osc.frequency.exponentialRampToValueAtTime(f0*.88,ctx.currentTime+t+.12);
    f.type='bandpass'; f.frequency.value=f0; f.Q.value=5;
    g.gain.setValueAtTime(.0001,ctx.currentTime+t);
    g.gain.exponentialRampToValueAtTime(.045,ctx.currentTime+t+.018);
    g.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+t+.14);
    osc.connect(f); f.connect(g); g.connect(ctx.destination);
    osc.start(ctx.currentTime+t); osc.stop(ctx.currentTime+t+.16);
  });
}
function cricketChirp(ctx){
  const freq=4000+Math.random()*650;
  const pulses=3+Math.floor(Math.random()*3);
  for(let i=0;i<pulses;i++){
    const t=i*.075, osc=ctx.createOscillator(), g=ctx.createGain();
    osc.type='triangle'; osc.frequency.value=freq;
    g.gain.setValueAtTime(.0001,ctx.currentTime+t);
    g.gain.exponentialRampToValueAtTime(.018,ctx.currentTime+t+.006);
    g.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+t+.045);
    osc.connect(g); g.connect(ctx.destination);
    osc.start(ctx.currentTime+t); osc.stop(ctx.currentTime+t+.05);
  }
}
function startCityAmbience(){
  try{
    cityAudioCtx=new(window.AudioContext||window.webkitAudioContext)();
    const ctx=cityAudioCtx;
    const master=ctx.createGain(); master.gain.value=.72; master.connect(ctx.destination);

    const ambience=ctx.createBufferSource();
    ambience.buffer=buildNoiseBuffer(ctx,5); ambience.loop=true;
    const low=ctx.createBiquadFilter(); low.type='lowpass'; low.frequency.value=isNightNow()?420:780;
    const ag=ctx.createGain(); ag.gain.value=isNightNow()?.018:.028;
    ambience.connect(low); low.connect(ag); ag.connect(master); ambience.start();

    cityAudioNodes={master,ambience,low,ag};

    // Continuous soft stream: filtered noise gives the river a natural texture.
    cityRiverNoiseSrc=ctx.createBufferSource();
    cityRiverNoiseSrc.buffer=buildNoiseBuffer(ctx,4); cityRiverNoiseSrc.loop=true;
    const rf=ctx.createBiquadFilter(); rf.type='bandpass'; rf.frequency.value=1050; rf.Q.value=.7;
    cityRiverGainNode=ctx.createGain(); cityRiverGainNode.gain.value=.018;
    cityRiverNoiseSrc.connect(rf); rf.connect(cityRiverGainNode); cityRiverGainNode.connect(master);
    cityRiverNoiseSrc.start();

    // Gentle wind layer, quieter at night.
    cityWindNoiseSrc=ctx.createBufferSource();
    cityWindNoiseSrc.buffer=buildNoiseBuffer(ctx,6); cityWindNoiseSrc.loop=true;
    const wf=ctx.createBiquadFilter(); wf.type='lowpass'; wf.frequency.value=420;
    cityWindGainNode=ctx.createGain(); cityWindGainNode.gain.value=.012;
    cityWindNoiseSrc.connect(wf); wf.connect(cityWindGainNode); cityWindGainNode.connect(master);
    cityWindNoiseSrc.start();

    function scheduleChirp(){
      if(!citySoundOn) return;
      if(isNightNow()) cricketChirp(ctx); else if(Math.random()<.8) birdChirp(ctx);
      cityChirpTimer=setTimeout(scheduleChirp,isNightNow()?(1100+Math.random()*1700):(1700+Math.random()*3800));
    }
    scheduleChirp();
    updateCityRainAudio();
  }catch(e){ cityAudioCtx=null; cityAudioNodes=null; }
}
function stopCityAmbience(){
  clearTimeout(cityChirpTimer);
  cityRainNoiseSrc=null; cityRainGainNode=null;
  cityRiverNoiseSrc=null; cityRiverGainNode=null;
  cityWindNoiseSrc=null; cityWindGainNode=null;
  if(cityAudioCtx){try{cityAudioCtx.close();}catch(e){}}
  cityAudioCtx=null; cityAudioNodes=null;
}
function updateCityRainAudio(){
  if(!cityAudioCtx)return;
  const ctx=cityAudioCtx;
  if(cityRainOn&&!cityRainNoiseSrc){
    cityRainNoiseSrc=ctx.createBufferSource(); cityRainNoiseSrc.buffer=buildNoiseBuffer(ctx,3); cityRainNoiseSrc.loop=true;
    const filter=ctx.createBiquadFilter(); filter.type='highpass'; filter.frequency.value=1200;
    cityRainGainNode=ctx.createGain(); cityRainGainNode.gain.value=.0001;
    cityRainNoiseSrc.connect(filter); filter.connect(cityRainGainNode); cityRainGainNode.connect(cityAudioNodes.master||ctx.destination);
    cityRainNoiseSrc.start(); cityRainGainNode.gain.exponentialRampToValueAtTime(.055,ctx.currentTime+1.6);
  }else if(!cityRainOn&&cityRainNoiseSrc){
    const src=cityRainNoiseSrc,g=cityRainGainNode; cityRainNoiseSrc=null;cityRainGainNode=null;
    g.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+1.1);
    setTimeout(()=>{try{src.stop()}catch(e){}},1200);
  }
}
function toggleCitySound(){
  citySoundOn = !citySoundOn;
  document.getElementById('citySoundBtn').textContent = citySoundOn ? '🔊' : '🔇';
  if(citySoundOn){ startCityAmbience(); } else { stopCityAmbience(); }
  // just update hint text, don't re-render whole scene
  const hint = document.getElementById('cityHint');
  if(hint) hint.textContent = citySoundOn ? '🔊 صدای طبیعت داره پخش می‌شه' : '🔇 صدا قطعه — برای پخش بزن';
}
...
function addPomoMinutes(kind, minutes){
  if(minutes<=0) return;
  const d = todayISO();
  ...
  addXP(Math.round(minutes * POMO_XP_PER_MIN * bonus));
  checkNewRecords();
}
...
function addManualPomoData(){
  ...
  if(!Array.isArray(DB.pomodoro.manualData))DB.pomodoro.manualData=[];
  DB.pomodoro.manualData.push({id:uid(),kind,minutes,date,note,createdAt:Date.now()});
  let manualXP=0;
  if(kind==='focus') manualXP=awardManualStudyXP(minutes,date);
  checkPerfectDay();
  save();
  document.getElementById('pomoManualMinutes').value='';document.getElementById('pomoManualNote').value='';
  toast(`✅ ${minutes} دقیقه ${kind==='focus'?'مطالعه':'استراحت'} ثبت شد${manualXP?` · +${manualXP} XP`:''}`);
}
function deleteManualPomoData(id){ DB.pomodoro.manualData=(DB.pomodoro.manualData||[]).filter(x=>x.id!==id); save(); toast('🗑️ ثبت دستی حذف شد'); }
function getManualPomoTotal(date,kind){return (DB.pomodoro.manualData||[]).filter(x=>x.date===date&&x.kind===kind).reduce((a,x)=>a+(x.minutes||0),0);}
function getPomoTotal(date,kind){return (DB.pomodoro.history[date]?.[kind]||0)+getManualPomoTotal(date,kind);}
function renderManualPomoData(){
  ...
  if(f)f.textContent=getManualPomoTotal(date,'focus')+'m';if(b)b.textContent=getManualPomoTotal(date,'break')+'m';
  ...
  list.innerHTML=arr.length?arr.slice(0,30).map(x=>`<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);"><div><div style="font-weight:800;">${x.kind==='focus'?'📚':'☕'} ${x.minutes} دقیقه</div><div class="muted" style="font-size:11.5px;">${x.date}${x.note?' • '+esc(x.note):''}</div></div><button class="btn ghost" style="padding:7px 10px;" onclick="deleteManualPomoData('${x.id}')">🗑️</button></div>`).join(''):'<div class="muted" style="padding:10px 0;">هنوز ثبت دستی نداری.</div>';
}
...
let eventActionCtx = null; // {eventId, dayIdx}
...
async function saveEvent(){
  ...
  if(reminder){
    if(!('Notification' in window)){
      reminder=false;
      toast('⚠️ این مرورگر اعلان رو پشتیبانی نمی‌کنه');
    }else if(Notification.permission!=='granted'){
      try{
        const permission=await Notification.requestPermission();
        if(permission!=='granted'){
          reminder=false;
          toast('🔕 اجازه اعلان داده نشد؛ رویداد بدون یادآور ذخیره شد');
        }
      }catch(_){
        reminder=false;
        toast('🔕 دسترسی اعلان ممکن نشد؛ رویداد بدون یادآور ذخیره شد');
      }
    }
  }

  const candidate={title,days,startMin,endMin};
  const conflict=findEventConflict(candidate,editingEventId);
  if(conflict){
...
async function checkEventReminders(){
  ...
  for(const ev of DB.events){
    if(!ev.reminder||!eventDays(ev).includes(day))continue;
    if(eventStartMinutes(ev)!==minute)continue;
    const key=reminderStorageKey(date,ev.id,day);
    if(localStorage.getItem(key)==='1')continue;
    localStorage.setItem(key,'1');
    const text=`${ev.title} — شروع شد`;
    try{
      if('serviceWorker' in navigator){
        const reg=await navigator.serviceWorker.getRegistration();
        if(reg) await reg.showNotification('🔔 یادآوری Life Planner',{body:text,tag:`lp-${ev.id}-${date}-${day}`,icon:'icon-192.png',renotify:false});
        else if('Notification' in window&&Notification.permission==='granted') new Notification('🔔 یادآوری Life Planner',{body:text});
      }else if('Notification' in window&&Notification.permission==='granted'){
        new Notification('🔔 یادآوری Life Planner',{body:text});
      }
    }catch(_){
      toast(`🔔 ${text}`);
    }
...
function startEventReminderScheduler(){
  checkEventReminders();
  setInterval(checkEventReminders,20000);
}
function deleteEvent(id){ DB.events = DB.events.filter(x=>x.id!==id); save(); }
...
function startEditEvent(){
  const ctx=eventActionCtx;if(!ctx)return;
...
async function checkLifePlannerUpdate(){
  try{
    ...
    if(v&&v!==LP_V64_APP_VERSION)showLifePlannerUpdate(v);
  }catch(_){}
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load', ()=>{
    navigator.serviceWorker.register('sw.js').catch(()=>{ /* silently ignore on file:// or unsupported hosts */ });
  });
}

...
/* ============ LIFE PLANNER v6.7 ENHANCEMENTS ============ */
const LP_V64_APP_VERSION = '6.7';
...
function renderSettings(){
  ...
  const ver=document.getElementById('settingsVersion'); if(ver) ver.textContent=LP_V64_APP_VERSION;
}
...
function showLifePlannerUpdate(v){
  if(lpV64UpdateShown || String(v)===LP_V64_APP_VERSION) return;
  ...
  document.getElementById('lpUpdateNow').onclick=async()=>{
    try{
      ...
      if('caches' in window){ const keys=await caches.keys(); await Promise.all(keys.filter(k=>k!=='life-planner-cache-v38').map(k=>caches.delete(k))); }
    }catch(_){ }
    location.reload();
  };
}

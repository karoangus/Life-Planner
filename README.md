...
- 🎨 **رنگبندی «برنامهی امروز» در داشبورد**: کارت تسکهای بخش «برنامهی امروز» در صفحهی داشبورد هم حالا مثل صفحهی تسکها همرنگ وضعیتشان میشود — «در حال انجام» سبز، «در صف» آبی، «متوقف» نارنجی و «شروع نشده» خاکستری (تسکهای انجامشده همان ظاهر ساده و کمرنگ قبلی را دارند)

---

## 🆕 تغییرات نسخه 14.0

- 🛡️ **Streak Shield گران‌تر شد**: قیمت از ۲۰۰ XP به **۵۰۰ XP** افزایش یافت
- 🛡️ **محدودیت Streak Shield**: حالا همیشه فقط **یک Streak Shield** می‌توانی داشته باشی و در هر هفته حداکثر **۳ عدد** می‌توانی بخری؛ سقف خرید هفتگی زیر آیتم در XP Shop نمایش داده می‌شود
- ⚔️ **مهلت باس هفتگی**: حالا فقط **۱۴ روز** فرصت داری باس را شکست بدهی؛ اگر شکست نخوری، باس از دست می‌رود و باید یک هفته صبر کنی تا باس جدید بیاید
- 🎨 **کارت تسک رنگی**: هر کارت تسک حالا رنگ خودِ وضعیتش را می‌گیرد (مثلاً «در حال انجام» سبز، «در صف» آبی، «متوقف» نارنجی)
- ✅ **بخش «تکمیل‌شده»**: بخش جدیدی بالای صفحه‌ی تسک‌ها اضافه شد که همه‌ی تسک‌های انجام‌شده را همان‌جا جمع می‌کند

---

## 🆕 تغییرات نسخه 13.2

- 📌 **رفع باگ نوار پایین و منوی «بیشتر»**: در نسخه‌ی ۱۳.۱ یک تغییر برای روان‌تر شدن اسکرول باعث شده بود نوار پایین (با دکمه‌ی «بیشتر») و منوی «بیشتر» دیگر به پایین صفحه‌ی گوشی چسبیده نمی‌ماندند و همراه با محتوا داخل صفحه می‌رفتند و اسکرول هم به‌هم می‌خورد. حالا نوار پایین و منوی «بیشتر» دوباره همیشه پایین صفحه محکم و ثابت هستند و اسکرول روان و درست کار می‌کند
- ⏱️ **حفظ بهینه‌سازی اسکرول ۱۳.۱**: لایه‌ی نور پس‌زمینه همچنان جدا از محتوا و روی لایه‌ی خودش رندر می‌شود، پس اسکرول نرم و بدون لگ می‌ماند

---

## 🆕 تغییرات نسخه 13.1

- ⏱️ **رفع باگ نشانگر «دقیقه X از Y» پومودورو**: این عدد قبلاً حین اجرای تایمر به‌روز نمی‌شد و فقط با مکث/ادامه حرکت می‌کرد؛ حالا هر ثانیه به‌صورت زنده و دقیق جلو می‌رود (هم در صفحه‌ی پومودورو و هم در حالت تمرکز تمام‌صفحه)
- 📋 **به‌روزرسانی زنده‌ی «برنامه‌ی امروز» در داشبورد**: اضافه، انجام یا ویرایش هر تسک بلافاصله روی داشبورد دیده می‌شود؛ دیگر لازم نیست اول به بخش تسک‌ها بروی و برگردی
- 📜 **اسکرول روان‌تر**: لایه‌ی نور پس‌زمینه از بدنه‌ی اسکرول‌شونده جدا شد تا حین اسکرول دوباره رنگ‌آمیزی نشود؛ حرکت صفحات نرم و بدون لگ است
- 🔔 **زنگ هشدار واقعی پایان جلسه‌ی پومودورو**: به‌جای «دینگ» کوتاه، یک زنگ دوضربی بلند و واضح (همراه با ویبره روی گوشی) پخش می‌شود تا پایان جلسه کاملاً مشخص باشد
...
- 🍅 **پومودورو v13**:
  - ⚙️ **پریست دیپ فوکوس به‌روزرسانی شد**: ۶۰ دقیقه فوکوس / ۱۵ دقیقه استراحت
  - ➕ **پریست سفارشی**: مدت فوکوس و استراحت دلخواهت را بساز و همیشه بین پریست‌ها در دسترس داشته باش
  - ⏭️ **دکمه‌ی «جلسه بعدی» مرحله‌به‌مرحله**: فوکوس → استراحت → فوکوس بعدی → استراحت و به همین ترتیب (دیگر مستقیم به فوکوس بعدی نمی‌پرد)
  - 📊 **میانگین فوکوس روزانه**: به آمار پومودورو اضافه شد
  - 🔔 **نوتیفیکیشن زنده**: شمارش معکوس تایمر به‌صورت زنده داخل اعلان به‌روز می‌شود
- ⏱️ **کوئست و درخت مهارت v13**: زمان انتظار بعد از هر کوئست از ۱۰ به ۲۰ دقیقه افزایش یافت؛ مهارت جدید «کوئست سریع» با ۳ سطح، هر سطح ۱ دقیقه از این انتظار کم می‌کند (۲۰ → ۱۷)
- 📌 **نوار دسترسی سریع v13**: به‌صورت پیش‌فرض یک ردیف با ۴ دکمه + «بیشتر»؛ در تنظیمات می‌توانی تا ۵ دکمه اضافه کنی تا نوار به دو ردیف وسط‌چین تبدیل شود

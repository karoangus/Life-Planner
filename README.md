...
<p align="center">
  <img src="https://img.shields.io/badge/نسخه-16.4-00e5ff?style=for-the-badge&logoColor=white" alt="Version 16.4" />
...
- **⚡ مأموریت‌های تصادفی روزانه (Quests):** پیشنهاد مأموریت‌های روزانه با درجات سختی متنوع (Easy تا Special)، پاداش فوری و تایمر اختصاصی؛ اگر شرایط انجامش رو نداشتی با دکمه‌ی «🚫 شرایط انجامش رو ندارم» بدون کسر XP ردش کن (مأموریت بعدی ۵ دقیقه بعد می‌رسه) — فقط «⏭️ ردش کن (با جریمه)» و رد کردن دستی XP کم می‌کنه.
...
1. فایل `app-version.json` ➔ کلید `"version"` (مثلاً `"16.4"`).
2. فایل `js/core.js` ➔ `const LP_APP_VERSION = '16.4';`
3. فایل `package.json` ➔ کلید `"version"` (مثلاً `"16.4.0"`).

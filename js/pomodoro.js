/* ============================================================
   Life Planner — Pomodoro (presets, focus sessions, ambient
   sounds via Web Audio, manual logging, notifications)
   Load order: after core.js + enhancements.js.
   Self-contained IIFE — does not add globals.
   ============================================================ */

/* ============ POMODORO v7.1 — unified manual logging ============ */
(function(){
  ...
  function recordSession(mode,minutes,meta={}){
    ...
    if(mode==='focus'){
      ...
      DB.pomodoro.sessionHistory.unshift({id:meta.manualId||uid(),date,mode:'focus',minutes,plannedMinutes:meta.plannedMinutes||minutes,taskId:meta.taskId||'',startedAt,endedAt,pauseCount:meta.pauseCount||0,score,note:meta.note||'',source:meta.source||'pomodoro'});
      DB.pomodoro.sessionHistory=DB.pomodoro.sessionHistory.slice(0,500);
      if(meta.taskId){ const t=(DB.tasks||[]).find(x=>String(x.id)===String(meta.taskId)); if(t){ t.pomodoroMinutes=(t.pomodoroMinutes||0)+minutes; t.lastPomodoroAt=endedAt; } }
      updateStreak();
    }
    const xp=addPomoMinutes(mode,minutes,date);
  ...
  try{ migrateLegacyManualPomoData(); updateStreak(); save(); }catch(_){ }
